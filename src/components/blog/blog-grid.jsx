...
const BlogGrid = () => {
    ...
    useEffect(() => {
     ...
     return (
    <div className={`${styles["blog-grid-area"]} pt-100 pb-100`}>
      <div className="container">
        <div className="row">
          <div className="col-12">
            <div className={`${styles["blog-grid-arrow"]} p-relative`}>
              
              <div className={`${styles["grid-next"]} d-none d-sm-block`}>
                <button>
              ...
              <div className={`${styles["grid-prev"]} d-none d-sm-block`}>
                <button>
              ...
              <Swiper 
                ...
                className={`${styles["swiper-container"]} ${styles["blog-grid-slider-active"]}`}
              > 
                {grid_slider_data.map((item, i) => (
                  <SwiperSlide key={i} className="swiper-slide"> 
                    <div  
                      className={`${styles["blog-grid-slider"]} ${styles["blog-grid-slider-bg"]} d-flex align-items-center blog-grid-slider-height`} 
                      style={{ backgroundImage: `url(${item.bg_img})` }}
                    >
                      <div className={styles["blog-grid-slider-wrapper"]}>
                        <div className={styles["blog-grid-slider-meta"]}>
                          <span className="child-one">{item.child_1}</span>
                          <span className="child-two">{item.date}</span>
                        </div>
                        <div className={styles["blog-grid-slider-title-box"]}>
                          <h4 className={styles["blog-grid-slider-title"]}>
                            <Link href="/blog-details">{item.title}</Link>
                          </h4>
                          <p>{item.des}</p>
                        </div>
                        <div className={`${styles["tp-blog-author-info-box"]} ${styles["blog-grid-avata-box"]} d-flex align-items-center`}>
                          <div className={styles["tp-blog-avata"]}>
                            <Image src={item.author_img} alt="theme-pure" />
                          </div>
                          <div className={styles["tp-blog-author-info"]}>
